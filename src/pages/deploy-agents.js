...
function renderGraph() {
  ...
  // Helper: match a domain key to a node (handles id, name, or partial match)
  const findNode = (key) => { if (!key) return null; const k = key.toLowerCase(); return nodes.find(x => x.id === key || x.id === k || x.name?.toLowerCase() === k || x.id.includes(k) || k.includes(x.id)); };
  if (S.debate?.relationships?.length) {
    S.debate.relationships.forEach(r => { const a = findNode(r.from), b = findNode(r.to); if (a && b) links.push({ from: a.id, to: b.id, type: r.type, reason: r.reason }); });
  } else if (S.debate) {
    (S.debate.conflicts || []).forEach(c => { const a = findNode(c.agent_a), b = findNode(c.agent_b); if (a && b) links.push({ from: a.id, to: b.id, type: 'conflicts', reason: c.incompatibility || '' }); });
    (S.debate.cross_domain_chains || []).forEach(ch => { (ch.chain || []).forEach((c, i, arr) => { if (i < arr.length - 1) { const a = findNode(c.domain), b = findNode(arr[i + 1].domain); if (a && b) links.push({ from: a.id, to: b.id, type: 'supports', reason: '' }); } }); });
  }
